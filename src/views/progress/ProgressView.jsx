...
import { bestSetPerSession } from '../../utils/progressionAnalyzer.js'
...
export function ProgressView() {
  ...
  async function loadExercises() {
    ...
    try {
      ...
      const strengthExercises = list.filter((e) => !e.is_cardio)
      setExercises(strengthExercises)
      if (strengthExercises.length > 0) {
        setSelectedId(strengthExercises[0].id)
      }
    } catch (err) {
  ...
  return (
    <section class="progress-view">
      ...
      {exercises.length === 0 ? (
        <p class="empty-state">Log some strength sets first — progress needs history to chart.</p>
      ) : (
        <>
          <select
